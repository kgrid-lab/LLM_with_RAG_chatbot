...
You may prepare and return a response with the code wrapping the function in ```function code ``` and the function call with input values (without assigning it to a variable) to execute it in ```line code ``` and include a context that could be used to prepare response for the user.
...
def process(text, conversation_history):
    ...
    if function:
        ...
        text = "I ran the code and the result is " +  str(result)+". use this value and provide the response with this value."
        full_context = get_full_context(conversation_history, text)
